...
const publishArticle = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
                        ...
                        try {
                            if (error)
                                ...
                                const updatedData: Partial<Article> = {
                                    ...
                                    isPublished: true,
                                    publicId: uploadedData[0].public_id,
                                    publishedImage: uploadedData[0].secure_url,
                                };
                                const published =
                                    await articleService.PublishArticle(
                                        fieldData.get('articleId'),
                                        fieldData.get('topicsId'),
                                        req.user_id,
                                        updatedData,
                                    );
                                ...
                                return res.status(httpStatusCode.OK).json({
                                    data: `${published.data.title} has been published successfully.`,
                                });
                            }
                        } catch (error) {
                            cloudinary.uploader.destroy(
                                uploadedData[0].public_id,
                            );
        ...
        bb.on('field', (fieldname, value) => {
            ...
            if (fieldname === 'articleId') {
                if (!isCuid(value)) {
                    return res
                        .status(httpStatusCode.BAD_REQUEST)
                        .json({ error: 'ArticleId must be a cuid.' });
                }
        ...
        bb.on('fieldsLimit', () => {
            return res
                .status(httpStatusCode.BAD_REQUEST)
                .json({ error: 'Unknown field is submited.' });
        });
